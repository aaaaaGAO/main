...
封装「读 Configuration.txt + FixedConfig.txt」与「按 section/option 取路径、输出名等」，
...
class GeneratorConfig:
    ...
    功能：
      - 统一加载 Configuration.txt 与 FixedConfig.txt。
      - 提供 get(section, key, fallback)、get_fixed(key, fallback)，优先 FixedConfig 再 Configuration。
    ...
    def load(self) -> "GeneratorConfig":
        """加载 Configuration.txt 与 FixedConfig.txt 到内存，可链式调用。
        无参数。使用 self.base_dir / self._config_path。返回: self，便于链式调用。
        """
        if self._config_path is None:
            self._config_path = find_config_path(self.base_dir)
    ...
    def get(self, section: str, key: str, fallback: str = "") -> str:
        """
        获取配置值：优先从 FixedConfig 取（key 或 key 小写），再从 Configuration 的 section/key 取。
    ...
    def raw_config(self) -> configparser.ConfigParser:
        """原始 Configuration 对象，供需要直接 .options() / .get() 的调用方使用。"""
        if not self._loaded:
            self.load()
