...
def load_mapping_context(cfg: Any, base_dir: str, config_path: str) -> Tuple[Any, Any]:
    """步骤 ②：加载 io_mapping 与 Configuration 枚举上下文；写入模块级供 legacy 使用。"""
    global _io_mapping_ctx, _config_enum_ctx
    _io_mapping_ctx, _config_enum_ctx = CINEntrypointSupport.load_mapping_context(
        cfg, base_dir=base_dir, config_path=config_path
    )
