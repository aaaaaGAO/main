...
def _apply_state_to_config(
    ...
    if any(
        ...
        # 点火循环配置：只要配置中已有 CENTRAL 段的 ign_* 或独立的 [IgnitionCycle] 段，就认为是“已配置”，允许根据 state 更新
        _has_ign_in_cfg = (
            (cfg.has_section("CENTRAL") and (cfg.has_option("CENTRAL", "ign_waittime") or cfg.has_option("CENTRAL", "ign_current")))
            or cfg.has_section("IgnitionCycle")
        )
        ign_wt = str(state.get("c_ign_waitTime") or "").strip()
        ign_cur = str(state.get("c_ign_current") or "").strip()
        if _has_ign_in_cfg and (state.get("c_ign_waitTime") is not None or state.get("c_ign_current") is not None):
