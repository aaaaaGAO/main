...
def get_project_root(reference_file: str | None = None) -> str:
    """获取工程根目录（含 Configuration.txt 的目录）。打包环境为 sys.executable 所在目录；开发环境从 reference_file 向上查找。
    ...
    for _ in range(10):
        if os.path.isfile(os.path.join(start, "Configuration.txt")):
            return start
...
def find_config_path(base_dir: str, filename: str = "Configuration.txt") -> str | None:
    """在 base_dir 及 base_dir/../release 下查找配置文件。
    参数: base_dir — 基准目录；filename — 配置文件名。
    返回: 绝对路径，找不到为 None。
    """
    candidates = [
        os.path.join(base_dir, filename),
        os.path.join(base_dir, "..", "release", filename),
    ]
    for p in candidates:
        p = os.path.abspath(p)
        if os.path.exists(p):
