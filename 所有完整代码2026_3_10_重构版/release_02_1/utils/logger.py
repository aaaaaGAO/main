...
def get_log_level_from_config(
    ...
    """从 Configuration.txt 指定 section 读取 log_level_min 并转为 logging 级别。
    参数: base_dir — 工程根目录；section — 配置节名，None 时使用当前运行域（set_run_domain）。
    返回: logging 等级（如 INFO/WARNING/ERROR）。
    """
    level_map = {
        "info": logging.INFO,
    ...
    try:
        ...
        cfg.read(os.path.join(base_dir, "Configuration.txt"), encoding="utf-8")
