...
class ConfigOption:
    ...
    INPUT_EXCEL = OPTION_INPUT_EXCEL
    IO_INPUTS = OPTION_IO_INPUTS
    DIDCONFIG_INPUT_EXCEL = OPTION_DIDCONFIG_INPUT_EXCEL
    OUTPUT_DIR = OPTION_OUTPUT_DIR
    OUTPUT_FILENAME = OPTION_OUTPUT_FILENAME
    SELECTED_SHEETS = OPTION_SELECTED_SHEETS
    LOG_LEVEL_MIN = OPTION_LOG_LEVEL_MIN
    UDS_ECU_QUALIFIER = OPTION_UDS_ECU_QUALIFIER
    CASE_LEVELS = OPTION_CASE_LEVELS
    CASE_PLATFORMS = OPTION_CASE_PLATFORMS
    CASE_MODELS = OPTION_CASE_MODELS
    CASE_TARGET_VERSIONS = OPTION_CASE_TARGET_VERSIONS
    RESETDID_INPUTS = OPTION_RESETDID_INPUTS
    RESETDID_INPUT_EXCEL = OPTION_RESETDID_INPUT_EXCEL
    CIN_INPUT_EXCEL = OPTION_CIN_INPUT_EXCEL
    SRV_EXCEL = OPTION_SRV_EXCEL
    SOA_SETSERVER_OUTPUT_FILENAME = OPTION_SOA_SETSERVER_OUTPUT_FILENAME
    SOA_DATATAB_OUTPUT_FILENAME = OPTION_SOA_DATATAB_OUTPUT_FILENAME
    UART_EXCEL = OPTION_UART_EXCEL
    XML_INPUT_EXCEL = OPTION_XML_INPUT_EXCEL
    IGN_WAITTIME = OPTION_IGN_WAITTIME
    IGN_CURRENT = OPTION_IGN_CURRENT
    LOGIN_USERNAME = OPTION_LOGIN_USERNAME
    LOGIN_PASSWORD = OPTION_LOGIN_PASSWORD
    C_PWR = OPTION_C_PWR
    C_RLY = OPTION_C_RLY
    C_IG = OPTION_C_IG
    C_PW = OPTION_C_PW
    IGNITION_CYCLE_WAIT_TIME = OPTION_IGNITION_CYCLE_WAIT_TIME
    IGNITION_CYCLE_CURRENT = OPTION_IGNITION_CYCLE_CURRENT


class UIStateKey:
    """UI 字段与前端状态键分组。"""

    UI_FIELD_RESETDID_EXCEL = UI_FIELD_RESETDID_EXCEL
    UI_FIELD_CIN_EXCEL = UI_FIELD_CIN_EXCEL
    UI_FIELD_SRV_EXCEL = UI_FIELD_SRV_EXCEL
    UI_FIELD_DIDCONFIG_EXCEL = UI_FIELD_DIDCONFIG_EXCEL
    UI_FIELD_CAN_INPUT = UI_FIELD_CAN_INPUT
    UI_FIELD_IO_EXCEL = UI_FIELD_IO_EXCEL
    UI_FIELD_OUT_ROOT = UI_FIELD_OUT_ROOT
    UI_FIELD_SELECTED_SHEETS = UI_FIELD_SELECTED_SHEETS
    UI_FIELD_LOG_LEVEL = UI_FIELD_LOG_LEVEL
    UI_FIELD_LEVELS = UI_FIELD_LEVELS
    UI_FIELD_PLATFORMS = UI_FIELD_PLATFORMS
    UI_FIELD_MODELS = UI_FIELD_MODELS
    UI_FIELD_TARGET_VERSIONS = UI_FIELD_TARGET_VERSIONS
    ...
    LR_RESETDID_EXCEL = STATE_KEY_LR_RESETDID_EXCEL
    DTC_RESETDID_EXCEL = STATE_KEY_DTC_RESETDID_EXCEL
    LR_CIN_EXCEL = STATE_KEY_LR_CIN_EXCEL
    DTC_CIN_EXCEL = STATE_KEY_DTC_CIN_EXCEL
    LR_DIDCONFIG_EXCEL = STATE_KEY_LR_DIDCONFIG_EXCEL
    DTC_DIDCONFIG_EXCEL = STATE_KEY_DTC_DIDCONFIG_EXCEL
    LR_CAN_INPUT = STATE_KEY_LR_CAN_INPUT
    DTC_CAN_INPUT = STATE_KEY_DTC_CAN_INPUT
    LR_IO_EXCEL = STATE_KEY_LR_IO_EXCEL
    DTC_IO_EXCEL = STATE_KEY_DTC_IO_EXCEL
    LR_OUT_ROOT = STATE_KEY_LR_OUT_ROOT
    DTC_OUT_ROOT = STATE_KEY_DTC_OUT_ROOT
    LR_SELECTED_SHEETS = STATE_KEY_LR_SELECTED_SHEETS
    DTC_SELECTED_SHEETS = STATE_KEY_DTC_SELECTED_SHEETS
    LR_LEVELS = STATE_KEY_LR_LEVELS
    DTC_LEVELS = STATE_KEY_DTC_LEVELS
    LR_PLATFORMS = STATE_KEY_LR_PLATFORMS
    DTC_PLATFORMS = STATE_KEY_DTC_PLATFORMS
    LR_MODELS = STATE_KEY_LR_MODELS
    DTC_MODELS = STATE_KEY_DTC_MODELS
    LR_TARGET_VERSIONS = STATE_KEY_LR_TARGET_VERSIONS
    DTC_TARGET_VERSIONS = STATE_KEY_DTC_TARGET_VERSIONS
    LR_SRV_EXCEL = STATE_KEY_LR_SRV_EXCEL
    DTC_SRV_EXCEL = STATE_KEY_DTC_SRV_EXCEL
    LR_LOG_LEVEL = STATE_KEY_LR_LOG_LEVEL
    DTC_LOG_LEVEL = STATE_KEY_DTC_LOG_LEVEL
    DTC_UDS_ECU_QUALIFIER = STATE_KEY_DTC_UDS_ECU_QUALIFIER
    DTC_IO_SELECTED_SHEETS = STATE_KEY_DTC_IO_SELECTED_SHEETS

    CENTRAL_CAN_INPUT = STATE_KEY_CENTRAL_CAN_INPUT
    CENTRAL_OUT_ROOT = STATE_KEY_CENTRAL_OUT_ROOT
    CENTRAL_LEVELS = STATE_KEY_CENTRAL_LEVELS
    CENTRAL_PLATFORMS = STATE_KEY_CENTRAL_PLATFORMS
    CENTRAL_MODELS = STATE_KEY_CENTRAL_MODELS
    CENTRAL_TARGET_VERSIONS = STATE_KEY_CENTRAL_TARGET_VERSIONS
    CENTRAL_SELECTED_SHEETS = STATE_KEY_CENTRAL_SELECTED_SHEETS
    CENTRAL_LOG_LEVEL = STATE_KEY_CENTRAL_LOG_LEVEL
    CENTRAL_UDS_ECU_QUALIFIER = STATE_KEY_CENTRAL_UDS_ECU_QUALIFIER
    CENTRAL_UART = STATE_KEY_CENTRAL_UART
    CENTRAL_SRV_EXCEL = STATE_KEY_CENTRAL_SRV_EXCEL
    CENTRAL_UART_COMM = STATE_KEY_CENTRAL_UART_COMM
    CENTRAL_PWR = STATE_KEY_CENTRAL_PWR
    CENTRAL_RLY = STATE_KEY_CENTRAL_RLY
    CENTRAL_IG = STATE_KEY_CENTRAL_IG
    CENTRAL_PW = STATE_KEY_CENTRAL_PW
    CENTRAL_IGN_WAIT_TIME = STATE_KEY_CENTRAL_IGN_WAIT_TIME
    CENTRAL_IGN_CURRENT = STATE_KEY_CENTRAL_IGN_CURRENT
    CENTRAL_LOGIN_USERNAME = STATE_KEY_CENTRAL_LOGIN_USERNAME
    CENTRAL_LOGIN_PASSWORD = STATE_KEY_CENTRAL_LOGIN_PASSWORD


class RuntimeKey:
    """运行期上下文键分组。"""

    CIN_CONFIG_PATH = CIN_RUNTIME_KEY_CONFIG_PATH
    CIN_CFG = CIN_RUNTIME_KEY_CFG
    CIN_INPUT_SHEET = CIN_RUNTIME_KEY_INPUT_SHEET
    CIN_INPUT_EXCEL_PATH = CIN_RUNTIME_KEY_INPUT_EXCEL_PATH
    CIN_MAPPING_EXCEL_PATH = CIN_RUNTIME_KEY_MAPPING_EXCEL_PATH
    CIN_SHEET_NAMES_STR = CIN_RUNTIME_KEY_SHEET_NAMES_STR
    CIN_OUTPUT_DIR = CIN_RUNTIME_KEY_OUTPUT_DIR
    CIN_OUTPUT_CIN_FILENAME = CIN_RUNTIME_KEY_OUTPUT_CIN_FILENAME
    CIN_IO_MAPPING_CTX = CIN_RUNTIME_KEY_IO_MAPPING_CTX
    CIN_CONFIG_ENUM_CTX = CIN_RUNTIME_KEY_CONFIG_ENUM_CTX

    XML_EXCEL_PATH = XML_RUNTIME_KEY_EXCEL_PATH
    XML_OUTPUT_XML_PATH = XML_RUNTIME_KEY_OUTPUT_XML_PATH
    XML_ALLOWED_LEVELS = XML_RUNTIME_KEY_ALLOWED_LEVELS
    XML_ALLOWED_PLATFORMS = XML_RUNTIME_KEY_ALLOWED_PLATFORMS
    XML_ALLOWED_MODELS = XML_RUNTIME_KEY_ALLOWED_MODELS
    XML_ALLOWED_TARGET_VERSIONS = XML_RUNTIME_KEY_ALLOWED_TARGET_VERSIONS
    XML_SELECTED_FILTER = XML_RUNTIME_KEY_SELECTED_FILTER


class DefaultValue:
    """默认值与固定标签分组。"""

    DID_CONFIG_FILENAME = DEFAULT_DID_CONFIG_FILENAME
    UDS_FILENAME = DEFAULT_UDS_FILENAME
    POWER_RELAY_CONFIG_FILENAME = DEFAULT_POWER_RELAY_CONFIG_FILENAME
    IGNITION_CYCLE_FILENAME = DEFAULT_IGNITION_CYCLE_FILENAME
    LOGIN_FILENAME = DEFAULT_LOGIN_FILENAME
    DOMAIN_LR_REAR = DEFAULT_DOMAIN_LR_REAR
    VALID_LOG_LEVELS = VALID_LOG_LEVELS
    RESETDID_INPUTS_VALUE_SUFFIX = RESETDID_INPUTS_VALUE_SUFFIX
