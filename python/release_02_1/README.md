...
| 文件 | 说明 |
|------|------|
| `config/Configuration.txt` | 主业务配置：输入/输出路径、IO 映射、各域参数、DID/CIN/UART 等 |
| `config/FixedConfig.txt` | 固定配置：部分输出文件名、路径等（可选，缺省用代码默认值） |
| `config/filter_options.txt` | 筛选项配置：等级、平台、车型、UDS_ECU 等下拉列表（可选，缺省下拉为空） |

程序会从 **工程根目录下的 config/** 中读取上述配置。开发时工程根目录 = 含 `config/Configuration.txt` 的目录；打包后 = EXE 所在目录。

### 3. 推荐目录（可选）

| 目录 | 说明 |
|------|------|
| `input/` | 存放输入 Excel（用例表、IO 映射、关键字表、DID/UART 矩阵等），在 Configuration.txt 中用相对路径引用 |
...
1. 确保根目录下存在 **config/Configuration.txt**（可从 `config` 目录内已有示例复制或修改）。
2. 在项目根目录执行：
   ```bash
   python app.py
   ```
...
3. 将 EXE 拷贝到目标机器后，在 **EXE 同级目录** 下建立 **config** 文件夹，并将 `Configuration.txt`、`FixedConfig.txt`、`filter_options.txt` 放入其中。
...
2. **配置路径与筛选项**  
   ...
   - 等级、平台、车型等筛选项若已配置 filter_options.txt，会出现在下拉框中。  
   - 可勾选要参与的 Sheet，配置会自动保存到 config/Configuration.txt。
