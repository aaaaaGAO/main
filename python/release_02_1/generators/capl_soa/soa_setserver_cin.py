...
class SOASetServerCinUtility:
    """SOA_StartSetserver.cin 的解析、过滤与渲染工具类（收口模块级散函数）。"""
    ...
    @staticmethod
    def extract_struct_name_and_first_field(
        grammar_text: str,
    ) -> tuple[str, str] | None:
        """解析负载语法中的结构体名和首字段名。
    ...
    @classmethod
    def locate_service_interface_header(cls, worksheet: Any) -> tuple[int, ColumnMapper]:
        ...
        for row_idx in range(1, max_row_scan + 1):
            header_row = cls.build_header_row_values(worksheet, row_idx, max_column)
            column_mapper = ColumnMapper(
                aliases=SERVICE_INTERFACE_COLUMN_ALIASES,
        ...
        error_message = (
            "Service_Interface 表中未找到有效表头行（需同时包含元组 ID / 周期 / 类型 / 负载参数语法相关列）"
        )
        logger.error(error_message)
        raise ValueError(error_message)

    @classmethod
    def locate_service_deployment_header(cls, worksheet: Any) -> tuple[int, ColumnMapper]:
        ...
        for row_idx in range(1, max_row_scan + 1):
            header_row = cls.build_header_row_values(worksheet, row_idx, max_column)
            column_mapper = ColumnMapper(
                aliases=SERVICE_DEPLOYMENT_COLUMN_ALIASES,
                required=required_fields,
            )
            if column_mapper.scan(header_row):
                return row_idx, column_mapper
    ...
    @classmethod
    def build_service_name_to_server_ecus_map(cls, workbook: Any) -> dict[str, set[str]]:
        """从 Service_Deployment 构建服务名到服务提供方集合映射。"""
        if SERVICE_DEPLOYMENT_SHEET not in workbook.sheetnames:
            error_message = (
                f"Excel 缺少工作表 «{SERVICE_DEPLOYMENT_SHEET}»：无法按服务名称与服务提供方执行过滤"
            )
            logger.error(error_message)
            raise ValueError(error_message)
        worksheet = workbook[SERVICE_DEPLOYMENT_SHEET]
        header_row_idx, column_mapper = cls.locate_service_deployment_header(worksheet)
        ...
        for row_idx in range(header_row_idx + 1, max_row + 1):
            ...
            if not server_ecu:
                continue
            service_key = cls.normalize_service_name_key(current_service_name)
            server_key = cls.normalize_server_ecu_key(server_ecu)
    ...
    def should_skip_service_by_uds_qualifier(
        ...
        qualifier_key = cls.normalize_server_ecu_key(uds_ecu_qualifier)
        if not qualifier_key:
            return False
        service_key = cls.normalize_service_name_key(service_name)
        if not service_key:
            return False
    ...
    def collect_setserver_call_lines(
        ...
        *,
        ...
        for row_idx in range(header_row_idx + 1, max_row + 1):
            ...
            if service_name:
                current_service_name = service_name
            if cls.should_skip_service_by_uds_qualifier(
                current_service_name,
                service_server_map,
                uds_ecu_qualifier,
            ):
            ...
            cycle_ms = cls.parse_cycle_ms_numeric(cycle_raw)
            if cycle_ms is None:
            ...
            parsed = cls.extract_struct_name_and_first_field(grammar_text)
            if not parsed:
                logger.warning("跳过行 %s：无法从负载语法解析结构体名或首字段", row_idx)
                continue
            struct_name, member_name = parsed
            sleep_token = cls.cycle_ms_to_sleep_token(cycle_ms)
    ...
    def resolve_srv_excel_absolute_path(cls, base_dir: str, domain_key: str) -> str:
        ...
        for option_name in (OPTION_SRV_EXCEL, *OPTION_SRV_EXCEL_CANDIDATES):
            srv_value = (
                generator_config.get_from_section(section_name, option_name, fallback="").strip()
            )
            if srv_value:
                break
        if not srv_value:
            raise ValueError(
                f"未配置 [{section_name}] srv_excel（服务通信矩阵），且请求未提供 excel_path"
            )
        ...
        *,
        ...
        try:
            if SERVICE_INTERFACE_SHEET not in workbook.sheetnames:
                ...
                logger.error(error_message)
                raise ValueError(error_message)
            service_server_map = SOASetServerCinUtility.build_service_name_to_server_ecus_map(workbook)
            worksheet = workbook[SERVICE_INTERFACE_SHEET]
            header_row_idx, column_mapper = SOASetServerCinUtility.locate_service_interface_header(worksheet)
            event_call_lines = SOASetServerCinUtility.collect_setserver_call_lines(
                worksheet,
                header_row_idx,
                column_mapper,
            ...
            method_call_lines = SOASetServerCinUtility.collect_setserver_method_lines(
                worksheet,
                header_row_idx,
                column_mapper,
                service_server_map=service_server_map,
        ...
        cin_text = SOASetServerCinUtility.render_cin_document(event_call_lines, method_call_lines)
        ...
        if not resolved_output_filename:
            fixed_base_dir = SOASetServerCinUtility.detect_base_dir_for_fixed_config(
                self._anchor_path,
                cached.normalized_excel_path,
            )
            resolved_output_filename = SOASetServerCinUtility.resolve_setserver_filename(fixed_base_dir)
