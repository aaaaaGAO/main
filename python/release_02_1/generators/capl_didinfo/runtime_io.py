...
_merged_cell_value、_parse_int_or_range、_parse_bit、_normalize_did、_normalize_field_data、
...
def find_header_row_and_cols(ws: Any) -> Tuple[int, dict]:
    ...
    for r in range(1, min(ws.max_row, 50) + 1):
        row_vals = {norm_str(ws.cell(r, c).value) for c in range(1, min(ws.max_column, 50) + 1)}
        if required.issubset(row_vals):
            col_map = {}
            for c in range(1, min(ws.max_column, 50) + 1):
                v = norm_str(ws.cell(r, c).value)
                if v in required:
                    col_map[v] = c
            return r, col_map
...
def find_variant_cols(ws: Any, header_row: int, variant_names: list[str]) -> dict[str, int]:
    ...
    canonical_to_original = {_norm_variant(v): v for v in variant_names}
    found: dict[str, int] = {}
    for r in range(1, header_row + 1):
        for c in range(1, ws.max_column + 1):
            cell_val = norm_str(ws.cell(r, c).value)
            key = _norm_variant(cell_val)
        ...
        if len(found) == len(variant_names):
            break
    missing = [v for v in variant_names if v not in found]
    if missing:
...
def _parse_int_or_range(s: str) -> Optional[Tuple[int, int]]:
...
def _parse_bit(s: str) -> Optional[Tuple[int, int, bool]]:
    ...
    if "-" in s:
        a, b = s.split("-", 1)
        return int(a.strip()), int(b.strip()), False
    try:
        i = int(s)
        return i, i, False
    except ValueError:
...
_DID_RE = re.compile(r"^\s*(?:0x)?([0-9A-Fa-f]+)\s*$")


def _normalize_did(did_str: str) -> Optional[str]:
    """规范化 DID：XXXX 或 0xXXXX -> 0xXXXX（大写）。"""
    if not did_str:
        return None
    m = _DID_RE.match(did_str.strip())
    if m:
        return f"0x{m.group(1).upper()}"
    return None


def _normalize_field_data(cell_val: Any) -> str:
...
def generate_from_sheet(
    ...
    *,
    ...
    for r in range(header_row + 1, ws.max_row + 1):
        ...
        normalized_did = _normalize_did(did_cell) if did_cell else None
        if normalized_did:
            current_did = normalized_did
            len_cell = merged_cell_value(ws, r, col_len)
            try:
        ...
        byte_val_raw = merged_cell_value(ws, r, col_byte)
        byte_raw = norm_str(byte_val_raw)
        byte_cell_val = ws.cell(r, col_byte).value
        byte_is_explicit_cell = byte_cell_val is not None
        bit_raw = norm_str(merged_cell_value(ws, r, col_bit))
        ...
        if not byte_raw and not bit_raw:
            row_has_any = any(norm_str(ws.cell(r, c).value) for c in range(1, min(ws.max_column, 50) + 1))
            if row_has_any:
                print(
                    f"[didinfo] ERROR Excel={excel_name} sheet '{sheet_name}', 车型 '{variant_name}', "
                    f"DID '{current_did}', 行 {r} 中 Byte/Bit 均为空，跳过该行。"
                )
            continue

        bit_parsed = _parse_bit(bit_raw) if bit_raw else (0, 7, True)
        if not bit_raw and byte_raw:
            print(
                f"[didinfo] INFO Excel={excel_name} sheet '{sheet_name}', 车型 '{variant_name}', "
                f"DID '{current_did}', 行 {r} 中 Bit 为空，已使用 ALL 代替。"
            )
        if bit_parsed is None:
            continue

        if byte_raw:
            byte_parsed = _parse_int_or_range(byte_raw)
            if byte_parsed:
            ...
            print(
                f"[didinfo] ERROR Excel={excel_name} sheet '{sheet_name}', 车型 '{variant_name}', "
                f"DID '{current_did}', 行 {r} 中 Byte 为空，跳过该行。"
            )
            continue
        ...
        field_data = _normalize_field_data(ws.cell(r, variant_col).value)
        ...
        if byte_is_explicit_cell and current_did is not None and current_len is not None:
            ...
            if prev_row is not None:
                print(
                    f"[didinfo] ERROR Excel={excel_name} sheet '{sheet_name}', 车型 '{variant_name}', "
                    f"DID '{current_did}', Length '{current_len}', 行 {r} 中 BytePosition={byte_pos} 重复（首次出现在行 {prev_row}）。"
                )
            else:
                prev_map[byte_pos] = r
