...
class GeneratedCasesBundleUtility:
    ...
    def process_excel_for_generated_case_cans(
        *,
        ...
        for (excel_file_path, sheet_name), cases in sheet_cases_map.items():
            GeneratedCasesBundleUtility.translate_sheet_cases_in_place(cases, translator)
            sheet_events = GeneratedCasesBundleUtility.build_sorted_sheet_events(
                cases,
                skip_events_map.get((excel_file_path, sheet_name), []),
            )
            log_progress_or_info(logger, f"  处理Sheet={sheet_name}")
            GeneratedCasesBundleUtility.log_sheet_events(logger, sheet_events)

            excel_name_inner = os.path.basename(excel_file_path)
            excel_basename = os.path.splitext(excel_name_inner)[0]
            can_filename = GeneratedCasesBundleUtility.per_sheet_can_filename(excel_basename, sheet_name)
            ...
            sheet_log_name = GeneratedCasesBundleUtility.per_sheet_log_basename(excel_basename, sheet_name)
    ...
    def build_master_can_lines(
        *,
        ...
        """
        拼装 Master `.can` 文本行：Encoding、公共 include、可选 SecOC/关键字 Clib，以及为各子表生成的
        `.can` 写入 ``#include``（与生成的文件名列表一致；目录前缀见下文代码中的 ``Testcases`` 相对路径）。
    ...
    def write_master_can_aggregate_file(
        *,
        ...
        master_lines = GeneratedCasesBundleUtility.build_master_can_lines(
