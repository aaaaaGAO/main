...
# -*- coding: utf-8 -*-
"""
UART 生成运行期 IO：路径/配置/Excel 解析/文本生成与日志。

供 runtime 导出，供 UARTGeneratorService 调用。
- ``UARTExcelParser``：通信矩阵表头定位与按 Msg ID 聚合读取。
- ``UARTRuntimeIOUtility``：运行路径、配置修复、RS232 段拼装与落盘。
"""
...
class UARTExcelParser:
    ...
    def read_uart_excel_data(
        ...
        *,
        ...
        """
        从 UART 通信矩阵 Excel 指定 Sheet 读取消息与信号，按 Msg ID 聚合。

        参数：
            excel_path：矩阵 Excel 路径。
            sheet_name：工作表名，默认 IVIToMCU。
            workbook：可选已打开工作簿，传入则复用且不关闭。

        返回：
            消息字典列表；Sheet 不存在或缺少必填列时返回 []。
        """
        ...
        for column_index, header_text in enumerate(header):
            normalized_header = UARTExcelParser.normalize_header_text(header_text)
        ...
        missing_cols = [column_name for column_name in required_cols if col_indices[column_name] is None]
        if missing_cols:
            display_names = [
                UARTExcelParser.REQUIRED_COLS_DISPLAY.get(column_name, column_name)
            ...
            print(f"  当前表头: {header}")
            print(
                "  必填列要求: Msg ID (Hex)、Message Name / 消息名称、DLC (Byte)、"
                "Signal Name / 信号名称、Array、Length (Bits)、Value Type"
            )
            if should_close_workbook:
                try:
                    workbook_obj.close()
...
class UARTRuntimeIOUtility:
    """UART 运行期路径、配置、内容生成与日志工具。"""

    @staticmethod
    def flush_std_streams() -> None:
        """在 UART 运行前刷新 stdout/stderr。"""
    ...
    def setup_logging(base_dir: str) -> logging.Logger:
        """初始化 generate_uart_from_config.log 并返回 logger。"""
    ...
    def read_uart_rs232_config(config: configparser.ConfigParser) -> dict | None:
        """从 [CENTRAL] 读取串口参数；无任何 uart_comm_* 时返回 None。"""
        ...
        return {
            "port": UARTRuntimeIOUtility.get_uart_comm_value(config, "uart_comm_port", ""),
            "baudrate": UARTRuntimeIOUtility.get_uart_comm_value(config, "uart_comm_baudrate", "115200"),
            "dataBits": UARTRuntimeIOUtility.get_uart_comm_value(config, "uart_comm_dataBits", "8"),
            "stopBits": UARTRuntimeIOUtility.get_uart_comm_value(config, "uart_comm_stopBits", "1"),
            "kHANDSHAKE_DISABLED": UARTRuntimeIOUtility.get_uart_comm_value(
                config, "uart_comm_kHANDSHAKE_DISABLED", "0"
            ),
            "parity": UARTRuntimeIOUtility.get_uart_comm_value(config, "uart_comm_parity", "0"),
            "frameTypeIs8676": UARTRuntimeIOUtility.get_uart_comm_value(
                config, "uart_comm_frameTypeIs8676", "0"
            ),
        }

    @staticmethod
    def read_frame_type_value(config: configparser.ConfigParser) -> str:
        """从 [PATHS] 读取 FrameTypeIs8676（0 或 1），缺省为 0。"""
    ...
    def resolve_io_paths(config: configparser.ConfigParser, base_dir: str) -> tuple[str, str, str]:
        """从配置解析 UART 输入 Excel、输出文件名与输出目录。"""
        ...
        if ivi_to_mcu_messages:
            ...
            for message in ivi_to_mcu_messages:
                ...
                for signal in message["signals"]:
                    lines.append(
                        f"{signal['signal_name']} {signal['array']} {signal['length']} {signal['value_type']}"
                    )
            lines.append("")
