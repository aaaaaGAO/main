...
class XMLGenerationUtility:
    ...
    def find_excel_files(input_path: str) -> list[str]:
        ...
        返回：
            list[str]：Excel 文件路径列表（已去重、排序，排除 ~$ 临时文件）。
        """
        ...
        excel_files = [
            ...
            if not os.path.basename(excel_file).startswith("~$")
        ]
    ...
    def to_pinyin_if_needed(
        ...
        *,
        ...
        if not XMLGenerationUtility.contains_chinese(text_value):
            return text_value
        if not _HAS_PYPINYIN or lazy_pinyin is None:
            ...
            return text_value
        pinyin_result = "".join(
            lazy_pinyin(text_value, errors=lambda non_chinese: list(non_chinese))
        )
    ...
    def parse_testcases_from_sheet(
        ...
        *,
        ...
        if missing_cols:
            ...
            err_msg = f"[错误] 工作表「{sheet_label}」缺少必填列：{'、'.join(missing_cols)}，跳过本表生成"
            if logger:
        ...
        for row_idx, row_vals in enumerate(
            ...
            case_id_value = (
                row_vals[case_id_col_idx] if len(row_vals) > case_id_col_idx else None
            )
            ...
            level_cell_val = (
                row_vals[level_col_idx]
                if level_col_idx is not None and len(row_vals) > level_col_idx
                else None
            )
            level_norm = (
            ...
            platform_cell_val = (
                row_vals[platform_col_idx]
                if platform_col_idx is not None and len(row_vals) > platform_col_idx
                else None
            )
            platform_norm = (
            ...
            model_cell_val = (
                row_vals[model_col_idx]
                if model_col_idx is not None and len(row_vals) > model_col_idx
                else None
            )
            model_norm = (
            ...
            case_type_str = (
                ...
                if case_type_col_idx is not None
                else "自动"
            )
            target_version_cell = (
            ...
            target_version_str = (
                ...
                if target_version_col_idx is not None
                else ""
            )
    ...
    def parse_testcases_from_excel(
        ...
        *,
        ...
        返回：
            (sheet_testcases_dict, stats_dict)
        """
        ...
        if allowed_sheet_names is not None and len(allowed_sheet_names) > 0:
            sheet_names = [sn for sn in sheet_names if sn in allowed_sheet_names]
        if selected_filter is not None:
            ...
            if excel_name_lower not in selected_filter:
                ...
                sheet_names = [
                    sn for sn in sheet_names if str(sn).strip().lower() in allowed_set
                ]
        ...
        for sheet_name in sheet_names:
            if cls.is_history_sheet_name(sheet_name):
                continue
            ...
            if header_row_idx is None or not header_vals:
                ...
                dump = cls.dump_sheet_head_preview(ws, max_rows=10, max_cols=25)
            ...
            if not found_group_col:
                ...
                warn_msg = (
                    ...
                    "但未识别到'功能模块/模块'列名，将回退到固定第3列读取功能模块。\n"
            ...
            sheet_seen_ids: dict = {}
            sheet_testcases, sheet_stats = cls.parse_testcases_from_sheet(
                ws,
                allowed_levels=allowed_levels,
                allowed_platforms=allowed_platforms,
                allowed_models=allowed_models,
            ...
            stats["filtered_by_target_version"] += sheet_stats.get(
                "filtered_by_target_version", 0
            )
    ...
    def generate_xml_content(
        ...
        *,
        ...
        返回：
            str：完整 XML 字符串（UTF-8，testmodule 根节点）。
        """
        lines: list[str] = []
        lines.append('<?xml version="1.0" encoding="utf-8" standalone="yes"?>')
        lines.append(
            f'<testmodule title="{cls.escape_xml(XML_TITLE)}" version="{cls.escape_xml(XML_VERSION)}">'
        )
        lines.append(f'\t<description>{cls.escape_xml(XML_DESCRIPTION)}</description>')

        excel_items = list(excel_files_dict.items())
        for _excel_idx, (excel_name, sheet_groups_dict) in enumerate(excel_items, start=1):
            excel_title = os.path.splitext(os.path.basename(excel_name))[0]
            if not excel_title or not excel_title.strip():
                excel_title = " "
            lines.append(f'\t<testgroup title="{cls.escape_xml(excel_title)}">')

            sheet_items = list(sheet_groups_dict.items())
            for _sheet_idx, (sheet_name, group_dict) in enumerate(sheet_items, start=1):
                sheet_title = " " if not sheet_name or not sheet_name.strip() else sheet_name
                lines.append(f'\t\t<testgroup title="{cls.escape_xml(sheet_title)}">')

                group_items = list(group_dict.items())
                for _group_key, testcases in group_items:
                    for testcase_item in testcases:
                        ...
                        testcase_name_pinyin = cls.to_pinyin_if_needed(
                            testcase_name, logger=logger
                        )
                        lines.append(
                            f'\t\t\t<capltestcase name="{cls.escape_xml(testcase_name_pinyin)}">\t'
                        )
