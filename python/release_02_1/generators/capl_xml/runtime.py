...
入口在 XMLEntrypointWorkflowUtility.run_generation。
"""
...
class XMLRuntimeState:
    """XML 运行期 logger 与日志管理器状态（进程内单例）。"""

    logger: ClassVar[Optional[logging.Logger]] = None
    parse_logger: ClassVar[Optional[logging.Logger]] = None
    log_manager: ClassVar[Optional[GeneratorLogger]] = None
...
class XMLRuntimeUtility:
    ...
    def stream_supports_tty(stream_obj: Any) -> bool:
        """判断流对象是否为 TTY。"""
        if not stream_obj or not XMLRuntimeUtility.stream_has_isatty(stream_obj):
            return False
        try:
    ...
    def load_runtime_config(
        ...
        if config.has_section(domain) or (
            ...
            try:
                fopts = FilterService.parse_shaixuan_config(base_dir)
            ...
            if allowed_target_versions is not None:
                print(
                    f"[xml] Target Version 过滤已启用: {sorted(allowed_target_versions)[:5]}"
                    f"{'...' if len(allowed_target_versions) > 5 else ''}"
                )
            else:
    ...
    def init_runtime_logging(base_dir: str) -> tuple:
        ...
        if XMLRuntimeState.log_manager is not None:
            XMLRuntimeState.log_manager.clear()
        XMLRuntimeState.log_manager = GeneratorLogger(
            base_dir,
            log_basename="generate_xml_from_can.log",
            logger_name="generate_xml_from_can",
            formatter_factory=XMLRuntimeUtility.create_blank_line_friendly_formatter,
            console=False,
        )
        logger = XMLRuntimeState.log_manager.setup()
        ...
        XMLRuntimeState.logger = logger
        try:
            XMLRuntimeState.parse_logger = get_testcases_parse_logger(base_dir)
        except Exception:
            XMLRuntimeState.parse_logger = None
    ...
    def get_quiet_skip() -> bool:
        ...
        return not XMLRuntimeUtility.stream_supports_tty(sys.stdout)

    @staticmethod
    def clear_run_logger() -> None:
        """清理 XML 运行日志上下文。"""
        clear_run_logger_impl(XMLRuntimeState.logger)
        XMLRuntimeState.logger = None
...
class XMLRuntimeAPI:
    ...
    def parse_testcases_from_excel(
        ...
        *,
        ...
        return runtime_io.XMLGenerationUtility.parse_testcases_from_excel(
            ...
            logger=XMLRuntimeState.logger,
            parse_logger=XMLRuntimeState.parse_logger,
            quiet_skip=XMLRuntimeUtility.get_quiet_skip(),
        )
    ...
    def generate_xml_content(excel_files_dict: dict) -> str:
        """根据分组后的数据生成 XML 文本。"""
        return runtime_io.XMLGenerationUtility.generate_xml_content(
            excel_files_dict,
            logger=XMLRuntimeState.logger,
        )
