...
# -*- coding: utf-8 -*-
"""
ResetDid_Value 表头解析与单 Sheet 生成逻辑。

供 ``capl_resetdid.runtime`` / ``ResetDidGeneratorService`` 调用：
- ``Field``：单字段结构（与位域计算配合）；
- ``ResetDidRuntimeIOUtility``：表头定位、合并单元格、DID/Byte/Bit 解析与生成片段。
"""
...
class ResetDidRuntimeIOUtility:
    """ResetDid Excel 解析与单 Sheet 生成工具。"""

    DID_PATTERN = re.compile(r"^\s*(?:0x)?([0-9A-Fa-f]+)\s*$")

    @staticmethod
    def find_header_row_and_cols(ws: Any) -> Tuple[int, dict]:
        """在工作表前 50 行内定位 Configure DID、Length (Bytes)、Byte、Bit 表头。"""
    ...
    def find_variant_cols(ws: Any, header_row: int, variant_names: list[str]) -> dict[str, int]:
        ...
        canonical_to_original = {
            ResetDidRuntimeIOUtility.norm_variant(variant_name): variant_name
        ...
        for row_index in range(1, header_row + 1):
            for column_index in range(1, ws.max_column + 1):
                ...
                item_key = ResetDidRuntimeIOUtility.norm_variant(cell_value)
    ...
    def normalize_did(did_str: str) -> Optional[str]:
        ...
        if not did_str:
            return None
        did_match = ResetDidRuntimeIOUtility.DID_PATTERN.match(did_str.strip())
        if did_match:
    ...
    def compute_positions_and_length(
        ...
        """计算 Field_BytePosition、Field_bitPosition、Field_FieldLength（bit）。"""
    ...
    def generate_from_sheet(
        ...
        *,
        ...
        """针对一个 Sheet 和一个车型列生成 ResetDid_Value 片段。"""
        header_row, cols = ResetDidRuntimeIOUtility.find_header_row_and_cols(ws)
        ...
        for row_index in range(header_row + 1, ws.max_row + 1):
            did_cell = StringUtility.norm_str(
                ResetDidRuntimeIOUtility.merged_cell_value(ws, row_index, col_did)
            )
            normalized_did = (
                ResetDidRuntimeIOUtility.normalize_did(did_cell) if did_cell else None
            )
            if normalized_did:
                current_did = normalized_did
                len_cell = ResetDidRuntimeIOUtility.merged_cell_value(ws, row_index, col_len)
                try:
                ...
                (
                    ...
                    last_written_did,
                    last_written_len,
                ) = ResetDidRuntimeIOUtility.flush_did_header(
                    out_lines,
            ...
            byte_val_raw = ResetDidRuntimeIOUtility.merged_cell_value(ws, row_index, col_byte)
            ...
            bit_raw = StringUtility.norm_str(
                ResetDidRuntimeIOUtility.merged_cell_value(ws, row_index, col_bit)
            )
            ...
            bit_parsed = (
                ResetDidRuntimeIOUtility.parse_bit(bit_raw) if bit_raw else (0, 7, True)
            )
            ...
            if byte_raw:
                byte_parsed = ResetDidRuntimeIOUtility.parse_int_or_range(byte_raw)
                if byte_parsed:
            ...
            byte_pos, bit_pos, length_bits = ResetDidRuntimeIOUtility.compute_positions_and_length(
                byte_parsed, (bit_start, bit_end), bit_is_all
            )
            field_data = ResetDidRuntimeIOUtility.normalize_field_data(
                ws.cell(row_index, variant_col).value
            )
            ...
            if byte_is_explicit_cell and current_did is not None and current_len is not None:
                ...
                if prev_row is not None:
                    print(
                        f"[resetdid] ERROR Excel={excel_name} sheet '{sheet_name}', 车型 '{variant_name}', "
                        f"DID '{current_did}', Length '{current_len}', 行 {row_index} 中 "
                        f"BytePosition={byte_pos} 重复（首次出现在行 {prev_row}）。"
                    )
                else:
            ...
            bit_str_tag = (
                "BitALL"
                if (not bit_raw or bit_is_all or bit_normalized.lower() == "all")
                else f"Bit{bit_normalized}"
            )
...
__all__ = ["ResetDidRuntimeIOUtility", "Field"]
