...
# -*- coding: utf-8 -*-
"""
ResetDid_Value 运行期：解析根目录、加载配置、初始化日志，供 ``ResetDidGeneratorService`` 无 hooks 调用。
...
from services.config_constants import (
    ...
    OPTION_RESETDID_INPUTS_CANDIDATES,
    OPTION_RESETDID_VARIANTS_CANDIDATES,
    OPTION_OUTPUT_DIR,
    SECTION_DTC,
    SECTION_LR_REAR,
)
...
_RESETDID_COALESCE_WHITELIST: set[tuple[str, ...]] = {
    tuple(reversed(OPTION_RESETDID_OUTPUT_DIR_CANDIDATES)),
    tuple(reversed(OPTION_RESETDID_VARIANTS_CANDIDATES)),
    tuple(reversed(OPTION_RESETDID_INPUTS_CANDIDATES)),
}


def resolve_base_dir() -> str:
    """解析 ResetDid 生成运行根目录（项目根）。"""
...
def coalesce_resetdid_whitelisted_options(
    ...
    *,
    ...
    """
    ResetDid 专用同节别名兼容读取：仅允许白名单键集做 coalesce，防止兜底范围失控。
    """
    if option_names not in _RESETDID_COALESCE_WHITELIST:
        raise RuntimeError(
            f"ResetDid 配置读取拒绝非白名单 coalesce：[{section_name}] {option_names}"
        )
...
def load_runtime_config(base_dir: str, domain: str | None = None) -> tuple:
    """读取 ResetDid 配置，返回 (config_path, output_path, variant_names, inputs)。"""
    gconfig = GeneratorConfig(base_dir).load()
    resetdid_inputs_keys_hint = " / ".join(reversed(OPTION_RESETDID_INPUTS_CANDIDATES))
    ...
    if effective_domain not in (SECTION_DTC, SECTION_LR_REAR):
        raise ValueError(
            f"ResetDid 仅支持 domain 为 {SECTION_LR_REAR!r} 或 {SECTION_DTC!r}，当前为 {effective_domain!r}"
        )
    ...
    if effective_domain == SECTION_DTC:
        ...
        sec = SECTION_DTC
        output_filename = gconfig.get_fixed("resetdid_output_filename") or "DIDInfo.txt"
        output_dir_resetdid = coalesce_resetdid_whitelisted_options(
            gconfig,
            sec,
            tuple(reversed(OPTION_RESETDID_OUTPUT_DIR_CANDIDATES)),
        ).strip()
        ...
        variant_raw = (
            (gconfig.get_fixed("resetdid_variants") or "").strip()
            or coalesce_resetdid_whitelisted_options(
                gconfig,
                sec,
                tuple(reversed(OPTION_RESETDID_VARIANTS_CANDIDATES)),
            )
        ...
        inputs_raw = coalesce_resetdid_whitelisted_options(
            gconfig,
            sec,
            tuple(reversed(OPTION_RESETDID_INPUTS_CANDIDATES)),
        ).strip()
        if not inputs_raw:
            raise RuntimeError(
                f"未配置 {LABEL_RESETDID_VALUE_CONFIG_TABLE}：请在 [DTC] {resetdid_inputs_keys_hint} 中配置 Excel 路径"
            )
    ...
    if not inputs_raw:
        raise RuntimeError(
            f"未配置 {LABEL_RESETDID_VALUE_CONFIG_TABLE}：请在 [{SECTION_LR_REAR}] {resetdid_inputs_keys_hint} 中配置 Excel 路径"
        )
    inputs = list(iter_input_specs(inputs_raw))
    return config_path, output_path, variant_names, inputs


def strip_resetdid_tee_msg(msg: str) -> str:
    ...
    返回：
        去掉 resetdid 等前缀后的消息。
    """
    msg = re.sub(r"^\[resetdid\]\s*(?:ERROR|INFO)\s*", "", msg, flags=re.I)
    msg = re.sub(r"^\[resetdid\]\s*(?:错误|警告)\s*:\s*", "", msg, flags=re.I)
...
def init_runtime(base_dir: str) -> tuple:
    """初始化 ResetDid 主日志与 Tee，返回 (logger, old_stdout, old_stderr)。"""
    global log_manager
    if log_manager is not None:
        log_manager.clear()
    log_manager = GeneratorLogger(
        base_dir,
        logger_name="generate_resetdid_from_excel",
        log_specs=[
            LogSpecConfig(
                subdir="parse",
            ...
            LogSpecConfig(subdir="gen", basename="generate_resetdid_from_excel.log"),
        ],
    ...
    if sys.__stdout__ is not None and sys.__stderr__ is not None:
        sys.stdout = TeeToLogger(
            ...
            warning_prefixes=("[resetdid] 警告", "[警告]", "[warn]"),
            msg_cleaner=strip_resetdid_tee_msg,
...
def generate_from_sheet(
    ...
    *,
    ...
    """按车型从单个 sheet 生成 ResetDid_Value 片段。
