...
- ExcelService / merged_cell_value : openpyxl 底层封装已迁入 infra.excel.workbook，此处统一导出
- ColumnMapper : 表头列映射（来自 infra.excel.header）
"""
...
def norm_str(raw_value) -> str:
    """安全地将任意值转为去首尾空白的字符串。参数: raw_value — 任意值。返回: str。"""
...
def nfc_normalize(raw_text: str) -> str:
    """对字符串做 Unicode NFC 归一化。参数: raw_text — 字符串。返回: 归一化后字符串。"""
    if not raw_text:
        return raw_text
