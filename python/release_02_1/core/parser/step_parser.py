...
本模块通过 `StepParser` 统一实现步骤文本到 CAPL 调用语句的解析，
覆盖普通关键字、Clib、SetRepeatKeyword、AutoIncreaseInVal、Keep*WithTime 等规则，
并保留 CIN 的“自动补 Step 前缀”兼容行为。
"""
...
class StepParser:
    ...
    def parse_line(
        ...
        *,
        ...
        start_idx = 0
        if len(tokens) >= 2 and tokens[0].lower() == "step" and tokens[1].lower() in cls._SPECIAL_CMDS:
            start_idx = 1
        ...
        if first_cmd == "setrepeatkeyword":
            ...
            try:
                ...
                raise StepSyntaxError(f"SetRepeatKeyword 次数无效: {tokens[val_idx]!r}") from exc
        ...
        if first_cmd == "clib":
            ...
            if clib_validator is not None and not clib_validator(original_clib_name):
            ...
            if extra_args:
                ...
                return ParseResult([f"  {capl_func}({', '.join(parsed)});"], original_line_full)
            return ParseResult([f"  {capl_func}();"], original_line_full)

        if first_cmd in cls._KEEP_TIME_CMDS:
            spec_kt = (
        ...
        if (not matched or spec is None) and mode_l == "cin" and match_tokens and match_tokens[0].lower() != "step":
        ...
        combined = " ".join([spec_kw_raw, spec_func_raw, spec_capl_raw]).strip().casefold().replace(" ", "_")
        use_config_enum = any(cw in combined for cw in cls._CFG_KWS)
        if use_config_enum and config_enum_ctx is not None:
            if not args:
