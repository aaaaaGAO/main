...
导出符号说明：
- IOMappingUtility.load_context_from_config(cfg, base_dir, ...) -> IOMappingContext
  从主配置文件加载 IO 映射表；解析失败抛出 IOMappingParseError。
- IOMappingContext : 按 Name 查 Path 等，供步骤翻译使用。
- ConfigEnumUtility.load_context_from_config(cfg, base_dir, ...) -> ConfigEnumContext
...
from core.translator.config_enum import ConfigEnumContext, ConfigEnumParseError, ConfigEnumUtility
from core.translator.io_mapping import IOMappingContext, IOMappingParseError, IOMappingUtility
