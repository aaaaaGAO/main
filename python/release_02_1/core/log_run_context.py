...
from core.caseid_log_dedup import LogDedupManager
...
class RunLogContext:
    """运行期日志目录与当前域的进程内上下文管理器（基于环境变量实现跨模块共享）。"""
    ...
    @classmethod
    def set_domain(cls, domain: str | None) -> None:
        """设置当前运行域（LR_REAR/CENTRAL/DTC），供解析表格日志等按域读取 log_level_min。"""
        if domain:
            os.environ[cls.ENV_RUN_LOG_DOMAIN] = str(domain).strip()
        else:
            os.environ.pop(cls.ENV_RUN_LOG_DOMAIN, None)

    @classmethod
    def get_domain(cls) -> str | None:
        """获取当前运行域；未设置时返回 None。"""
        return (os.environ.get(cls.ENV_RUN_LOG_DOMAIN) or "").strip() or None
    ...
    def reset(cls) -> None:
        ...
        - 清除用于缓存本次“生成任务”日志目录的环境变量，
          使得下一次调用 ensure_dirs 时一定会基于最新时间戳创建新目录。
        - 清空 caseid_clean_dup 的去重集合，确保二次点击时能写入新日志目录。
        - 典型用法：在每次任务入口（如 CANEntrypointWorkflowUtility.run_generation）开始时调用，
          确保每次点击“生成”都会落在全新的 log_YYYYMMDD_HHMMSS 目录中。
        """
        for item_key in (
            cls.ENV_RUN_LOG_ROOT,
            cls.ENV_RUN_LOG_GEN_DIR,
            cls.ENV_RUN_LOG_PARSE_DIR,
            cls.ENV_RUN_LOG_DOMAIN,
        ):
            os.environ.pop(item_key, None)
        try:
            LogDedupManager.reset()
        except Exception:
            pass

    @classmethod
    def ensure_dirs(cls, base_dir: str, *, force_new: bool = False) -> RunLogDirs:
        ...
        root_env = os.environ.get(cls.ENV_RUN_LOG_ROOT, "").strip()
        gen_env = os.environ.get(cls.ENV_RUN_LOG_GEN_DIR, "").strip()
        parse_env = os.environ.get(cls.ENV_RUN_LOG_PARSE_DIR, "").strip()
        if (
            not force_new
            and root_env
            and gen_env
        ...
        os.environ[cls.ENV_RUN_LOG_ROOT] = root_dir
        os.environ[cls.ENV_RUN_LOG_GEN_DIR] = gen_dir
        os.environ[cls.ENV_RUN_LOG_PARSE_DIR] = parse_dir
